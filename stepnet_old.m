function [value]=stepnet(steps,mode,com,motor,safety)
%clc
%clear all
s = instrfind;
if ~isempty(s)
...
fprintf(s,'%s\r', 's r0xc8 256');
pause(.2)
readout = fscanf(s,'%c',s.BytesAvailable);
%set position
posi=['s r0xca ', num2str(round(steps))];
fprintf(s,'%s\r', posi);
pause(.2)
%set velocity 0.1counts/second   1count= 0.25mm/1000steps 
readout = fscanf(s,'%c',s.BytesAvailable);
fprintf(s,'%s\r', 's r0xcb 5000000');
pause(.2)
readout = fscanf(s,'%c',s.BytesAvailable);
%set accelertion 10count/second^2
fprintf(s,'%s\r', 's r0xcc 100000');
pause(.2)
readout = fscanf(s,'%c',s.BytesAvailable);
%set accelertion 10count/second^2
fprintf(s,'%s\r', 's r0xcd 100000');
pause(.2)
readout = fscanf(s,'%c',s.BytesAvailable);
%enable amplifier
fprintf(s,'%s\r', 's r0x24 31')
pause(.2)
readout = fscanf(s,'%c',s.BytesAvailable);
%execute move 
fprintf(s,'%s\r', 't 1')
